...
st.set_page_config(
    page_title="Nexus Pro v4.4: Bootcamp A1→C1",
    page_icon="🦅",
    layout="wide"
)

# --- INICIALIZACIÓN SEGURA (SOLUCIONA EL ATTRIBUTE ERROR) ---
...
# ==================== 2. CURRÍCULO Y CONTENIDO ====================
CURRICULO = {
    "A1.1": {
        "tema": "Saludos y Presentaciones",
        "explicacion": """**👨‍🏫 CLASE MAGISTRAL 1.1: El Arte de Presentarse**
1. **Conexión de Sonidos:** Di "Mynameis" como una sola palabra.
2. **La 'H' aspirada:** En 'Hello', suena como un suspiro frío.
3. **Entonación:** Las preguntas suben de tono al final.""",
        "frases": [
    ...
    "A1.2": {
        ...
        "explicacion": "El motor del inglés: I am, You are, He/She is.",
        "frases": [
...
def obtener_feedback_profesor(objetivo, dicho):
    ...
    prompt = f"Profesor de inglés: El alumno dijo '{dicho}' para la frase '{objetivo}'. Explica en español corto qué sonido falló."
    try:
...
# --- FASE 3: EXAMEN ---
    ...
    st.subheader(f"📝 Examen: {st.session_state.pregunta_actual + 1}/{len(config['examen'])}")
    ...
    if "res_examen" in st.session_state:
        ...
        if res["prec"] >= 75: st.success(f"✅ Correcto")
        else: st.error(f"❌ Error. Dijiste: '{res['texto']}'")

        if st.button("➡️ CONTINUAR"):
            if st.session_state.pregunta_actual < len(config['examen']) - 1:
                ...
                if st.session_state.respuestas_correctas >= 4:
                    ...
                    st.error("Examen no aprobado. Repasemos."); st.session_state.fase = "explicacion"
